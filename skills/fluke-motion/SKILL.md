...
- [showcase/motion-showcase.html](showcase/motion-showcase.html) — **"Motion in Context"**: 15 technique sections, each *explained* (what moves / how / why) and shown inside a **real component** — pricing block, feature selector, testimonial drag-slider, editorial sticky-pin story, nav, sign-up form, tilt cards, kinetic headline, scroll-reactive band, loading states, toasts, cursor-reveal, generative background, page transitions, FAQ. The "show the implementation in a real case, section by section" companion to the abstract catalog. One clock, native scroll, reduced-motion toggle, light-locked.
