---
name: fluke-motion
description: Full-spectrum UI motion design assistant for product designers — from basic micro-interactions to top-tier complex animation (parallax, scroll-driven, scrollytelling, spring physics, shared-element transitions, SVG/shader effects, WebGL heroes, 3D tilt cards, gesture-driven motion), on web AND mobile (React Native Reanimated, Flutter). When working inside a project, it first scans the codebase (stack, components, existing motion tokens and animation libraries) so every implementation matches the project's conventions. Three modes — write dev-ready motion specs (easing, duration, trigger), generate runnable animation prototypes (CSS, Framer Motion, GSAP, Reanimated, Flutter, or whatever fits the project), and critique existing motion against animation principles, accessibility, and performance. Use this whenever the user asks about animation, transitions, micro-interactions, hover/loading states, skeletons, toasts, preloaders, page transitions, "how should this move," easing curves, motion handoff docs, parallax, scroll effects, springs, or wants an animated prototype. Trigger even without the word "motion" — e.g. "make this button feel more alive," "the modal feels janky," "I want an Apple-style scroll section," "spec out the loading state," "animate my logo," "bikin efek hover keren."
---
